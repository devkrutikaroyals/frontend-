...
const MasterDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (activeSection === "products") {
      axios
        .get("https://newmedizon.onrender.com/api/products/list", {
          headers: {
            Authorization: `Bearer ${token}`, // Correct: Use backticks
          },
        })
        .then((response) => setProducts(response.data))
        .catch((error) => console.error("Error fetching products:", error));
  ...
  return (
    <div className="dashboard-container">
      {/* Sidebar */}
      <div className={`sidebar ${isCollapsed ? "collapsed" : ""}`}> {/* Correct: Use backticks */}
        <div className="sidebar-logo">
          <img src={logo} alt="Company Logo" />
        </div>
        <button className="toggle-btn" onClick={toggleSidebar}>
      ...
      {/* Main Content */}
      <div className={`content ${isCollapsed ? "collapsed" : ""}`}> {/* Correct: Use backticks */}
        <div className="header">
          <h2>Welcome to the Admin Panel</h2>
          <div className="notification-container">
            <FaBell className="notification-icon" onClick={toggleNotifications} />
